...
// Valeur que l'on va écrire
static uint8_t val_write = 5;
...
int main(void)
{
	...
	/* Si la connexion bluetooth est établie, on écrit une valeur toutes les 5 secondes*/
	while (1) {
		...
		if (info_conn) {
			...
			/* Incrémentation de la valeur à envoyer*/
			val_write += 1;
			...
			/*Fonction pour écrire un attribut du périphérique, sans réponse de confirmation, et en déclenchant un callback*/
			/*
			if (bt_gatt_write_without_response_cb(info_conn, 18, &val_write, sizeof(val_write), 0, complete_func_write, NULL) == 0) {
				printk("Write succesfull\n");
			} else {
				printk("Write failed\n");
			}
